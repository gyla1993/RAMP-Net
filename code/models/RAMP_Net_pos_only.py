...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.station_null = nn.Parameter(torch.randn(1, self.d_model, 1, 1))  # for stations
        self.grid_null = nn.Parameter(torch.randn(1, self.d_model, 1,1, 1))  # for grids
    ...
    def forward(self,obs_his, era_his, cobs, cera, mask, datee):
        ...
        Be, De, Ne, Ke, Le = era_p.shape

        grid_null_expanded = self.grid_null.expand(Be, -1, Ne, Ke, Le)  # (B, D, N, K, L)
        era_t_gc = torch.cat([grid_null_expanded, era_p], dim=1)  # B D+36 N K L
        ...
        Bo,  Co,No,Lo = obs_his.shape  # BNTC
        sta_null_expanded = self.station_null.expand(Bo, -1, No, Lo)  # (B, D, N, L)
        obs_t_gc = torch.cat([sta_null_expanded, cobs_enc], dim=1)  # Q   [B D+16 N L]
        ...
        for i in range(self.layer_num):
            obs_t = torch.cat([sta_null_expanded, date_enc], dim=1)  # QK [B D+18 N L]
            ...
            obs_s = torch.cat([sta_null_expanded, cobs_enc], dim=1)  # QK [B D+12 N L]
