...
class Model(nn.Module):
    ...
    def forward(self,obs_his, era_his, cobs, cera, mask, datee):
        ...
        #print(obs_t.shape)
        era_t=self.o_e_tcn(era_his)#B D N L"""
        ...
        for i in range(self.layer_num):
            obs_t=torch.cat([obs,date_enc],dim=1)# QK [B D+18 N L]
        ...
        """attn_processed = atten_N2.reshape(B, N, L, self.n_heads, 4)#BNL H 1 4
        attn_for_test = attn_processed.mean(dim=(2, 3))  # [B, N, 4]
        attn_processed_self = atten_N.reshape(B, L,self.n_heads, N,N)#BL H N N
        attn_for_test_N = attn_processed_self.mean(dim=(0, 1))  #  [B, N, N]
        """
